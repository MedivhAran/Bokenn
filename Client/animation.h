...
public:
    ...
    void add_frame(IMAGE* image, int num_h)     //裁剪图片，变成帧
    {
    ...
    void add_frame(Atlas* atlas)
    {
        ...
        {
            IMAGE* image = atlas->get_image(i);     //获取图集中第i张图片
            int width = image->getwidth();              //获取图片宽
            int height = image->getheight();            //获取图片高

            Rect rect_src;                              //矩形裁剪
            rect_src.x = 0, rect_src.y = 0;
            rect_src.w = width, rect_src.h = height;

            frame_list.emplace_back(image, rect_src);       //加入序列帧
        }
    }
